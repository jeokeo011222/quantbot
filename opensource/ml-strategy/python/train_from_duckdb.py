...
class FeatureEngineer:
    ...
    def calculate_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 13. 按规范顺序输出（缺失列补 0），最终 fillna(0) 与 Go 端一致
...
def train_model(
    ...
    # 特征工程（列顺序固定为 FEATURE_COLS）
    print(f"\n  [{model_type}] Calculating features...")
    engine = FeatureEngineer()
    df_features = engine.calculate_features(df)
    df_features = df_features.reindex(columns=FEATURE_COLS)
    print(f"  [{model_type}] Features: {len(df_features)} rows x {len(df_features.columns)} cols")

    # 创建标签
    print(f"  [{model_type}] Creating labels (forward={forward_days}d, threshold={threshold})...")
    labels = engine.create_labels(df, forward_days, threshold)

    # 对齐特征与标签（特征窗口期 NaN 由 calculate_features 已 fillna(0)，标签尾部 NaN 丢弃）
    valid_idx = labels.dropna().index
    X = df_features.loc[valid_idx].astype(np.float64)
    y = labels.loc[valid_idx].astype(int) + 1  # -1→0, 0→1, 1→2

    print(f"  [{model_type}] Valid samples: {len(X)}")
    print(f"  [{model_type}] Label dist: sell={int((y == 0).sum())}, hold={int((y == 1).sum())}, buy={int((y == 2).sum())}")

    if len(X) < 1000:
        return {"error": f"样本不足: {len(X)} < 1000"}

    # 数据划分（保持按时间顺序的前后划分更贴近实盘，但为与旧模型可比沿用随机分层）
...
def main():
    ...
    for mt in model_types:
        name_suffix = {"xgboost": "xgb", "lgbm": "lgbm", "rf": "rf", "logistic": "logistic", "mlp": "mlp"}[mt]
        model_name = f"astock_{name_suffix}_v1"
        print(f"\n{'#'*60}\n# 训练 {mt}  ->  {model_name}\n{'#'*60}")
        res = train_model(
            full_df,
            model_type=mt,
