...
| 🛡️ **三层风控**<br>规则门 + AI 评审 + 熔断 + 单票 30%/回撤 10% 统一 + 执行端买入硬门控 | 🧪 **量化回测**<br>成本模型 + 样本池胜率校准 + **多策略并行** |
| 📊 **投资组合中心**<br>MVO / 风险平价 / 风险预算三算法最优配置（纯研究） | 🚪 **CIO 建仓硬门控**<br>共享阻断开关（拦买不拦卖） | 🧾 **决策合规防线**<br>股票池白名单 + 信号来源留痕 |
...
- **口语化问题**：如"行情走弱时怎么应对"，Planner 识别后直接影响风险等级。
- **单票上限按风险等级定**：保守 15% / 均衡 20% / 成长 30%。
- **小额成长方案走激进配置**：弱市现金约 10%、保留至少 3 只候选，避免资金闲置。

### 🧾 交易活动流

- 成交/活动流补齐展示**股票代码与名称**，避免同名时间戳被误合并。
...
- **多策略并行回测**：样本池 K 线仅加载一次，各策略通过有界 worker 池并行回测，显著缩短批量测试耗时。
...
- **选股硬筛选**：流动性因子分硬下限（不足即剔除）+ 流通市值 20~200 亿区间过滤（剔除微盘/超大盘）。
...
| 📊 **投资组合中心** | 基于真实持仓/日K协方差，MVO / 风险平价 / 风险预算三算法最优配置（纯研究，不影响投资决策） | |
...
基于**真实持仓 + 日K协方差**，一键给出三种经典配置方案：

- **MVO 均值方差** · **风险平价** · **风险预算** —— 三种算法的最优配置对比。
- 纯研究定位：仅供 CIO / 投资规划师 / Quant 参考，**不直接影响投资决策**。
- 能力已工具化为 `optimize_portfolio`，前端与智能体共享同一套数据与算法。
...
| **风控硬限制统一** | 单票权重上限 30% + 组合最大回撤容忍 10%；风控师运行时、策略引擎、风险中心三方代码一致 |
| **CIO 建仓硬门控** | "暂停建仓"共享阻断开关（独立于紧急停止，只拦买入、放行卖出），覆盖所有买入成交路径；退潮/仓位系数过低时不生成新买入订单 |
| **执行端硬风控（买入）** | 单票集中度上限（默认 30%）+ 买入流动性拥挤度门控（近 20 日换手 + 名义成交额阈值），超限拒绝下单 |
| **决策合规防线** | 买入须过股票池白名单（不在池剔除、池空即拒绝）；每笔买入记录策略信号来源（SignalSource） |
...
- 🔒 **单实例运行**：启动自动检测，仅允许一个进程实例
- 🔔 **系统托盘常驻**：点 × 不退出而是隐藏到系统托盘，托盘图标左键恢复、右键菜单可恢复/退出，后台工作流持续运行且随资源管理器重启自动重建图标
